...
def read_tokenizer(
    lang_id, g2p_model="latest", device=None, use_lexicon=True, jit=False
):

    lang_id = normalize_lang_id(lang_id)

    if lang_id in lang2tokenizer:
        return lang2tokenizer[lang_id](
            lang_id=lang_id, g2p_model=g2p_model, device=device, use_lexicon=use_lexicon, jit=jit
        )
    else:
        return read_g2p_tokenizer(
            lang_id=lang_id, g2p_model=g2p_model, device=device, jit=jit
        )
